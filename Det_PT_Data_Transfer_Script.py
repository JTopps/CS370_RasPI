...
#Print out the correct way to call the script in the case  of
def usage():
...
#Data is a list of cadets present, master is the master list
...
def get_absent_list():
    ...
    data = parse_scannner_data("Data.xls")

    for id in data:
        if not match_reverse(id, master):
            invalid.append(id)
...
#Read in scanner data from the event in question
def parse_scannner_data(filename):
    try:
...
def get_file_modify_time():
    ...
    date = datetime.fromtimestamp(fileTime).strftime('%Y-%m-%d %H:%M:%S')
    return date

#Decides whether event recorded was PT or LLAB based on the time the file was
#last modified
